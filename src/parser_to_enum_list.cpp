...
Lib_parser::List::List(List &&other)
{
    this->m_ptr_root = other.m_ptr_root;
    other.m_ptr_root = nullptr;
}
// ---------------------------------------------------------------------------------
Lib_parser::List & Lib_parser::List::operator=(List &&other)
{
    this->m_ptr_root = other.m_ptr_root;
    other.m_ptr_root = nullptr;
    return *this;
...
// ---------------------------------------------------------------------------------
// definitions for the type "parser"
Lib_parser::List Lib_parser::parser::parse_from(data_t src_data)
{
    // set buffer for getting some tokens
    m_tstream.set_buffer(src_data);
    // set tree nodes
    while(m_tstream)
        m_result.add(m_tstream.get());
    // enumerate it
    Node *ptr_node {m_result.get_head()};
    m_enumerator.inc_number(ptr_node);
    while(ptr_node)
    {
        while(ptr_node->m_ptr_right)
        {
            ptr_node=ptr_node->m_ptr_right;
            m_enumerator.add_number(ptr_node);
        }
