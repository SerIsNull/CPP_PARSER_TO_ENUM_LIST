...
// ---------------------------------------------------------------------------------
// Name: test case #4. Test parser
// Desc: this test for tesing parser type which coordinate process of parsing.
// good result: the number and the data of enumerate Node into the List are equal to number and data of valid Node.
// bad result: the number and the data of enumerate Node into the List are not equal to number and data of valid Node.
using ParserTest = parser_test::ParserTest;
TEST_F( ParserTest, PutSrcDataResultEnumerateList)
{
    Lib_parser::parser prs;
    Lib_parser::List ls {prs.parse_from(m_source_data)};
    Lib_parser::data_t expected {};
    Lib_parser::data_t actual {};
    Lib_parser::Node *ptr_node {ls.get_head()};
    std::size_t num_nodes{0};
    while(ptr_node)
    {
        const auto & [num, data] = m_valid_enum_nodes[num_nodes];
        expected = num + data;
        actual = ptr_node->m_number+ptr_node->m_data;
